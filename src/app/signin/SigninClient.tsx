...
export default function SigninClient({ redirectTo }: { redirectTo: string | null }) {
  ...
  useEffect(() => {
    ...
    const syncSession = async () => {
      try {
        ...
        // No session — the localStorage hydration already covered legacy state.
      }
    };
    syncSession();
    return () => {
    ...
    try {
      ...
      const { data, error } =
        authMode === "signin"
          ...
          : await supabase.auth.signUp({ email, password: inputPassword });
      ...
      persistSession(user.email || email, user.id ?? null);
